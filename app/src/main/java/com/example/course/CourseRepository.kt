...
class CourseRepository(
    ...
    suspend fun fetchSubjectSpecificLessons(
        programId: String,
        phaseId: String,
        subjectId: String,
        ...
        val qWithoutContentType = """
            query SubjectSpecificLessons(${'$'}program_id: String!, ${'$'}phase_id: String!, ${'$'}subject_id: String!) {
              studentSpecificLessons(program_id: ${'$'}program_id, phase_id: ${'$'}phase_id, subject_id: ${'$'}subject_id) {
                data {
                  access_level
                  title
                  id
        ...
        try {
            if (!contentType.isNullOrBlank()) {
                ...
                val data = response.data?.studentSpecificLessons?.data
                if (!data.isNullOrEmpty()) return data
            } else {
                val variables = mapOf<String, Any>(
                    "program_id" to programId,
                ...
                val query = GraphQlQuery(
                    operationName = "SubjectSpecificLessons",
                    query = qWithoutContentType,
                    variables = variables
                )
                val response = apiService.getStudentLessons(query)
                val data = response.data?.studentSpecificLessons?.data
            ...
            android.util.Log.w("CourseRepository", "fetchSubjectSpecificLessons failed: ${e.message}")
        }
        return emptyList()
    }
